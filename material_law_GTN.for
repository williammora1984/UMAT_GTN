...
c==================================================================
      ...
      subroutine kGTN (eps6,Deps6,sdvl,ttype,inputmat,sig6,A66,sdvup)
       !eps,epsn,epsen
        use tensor_operations

        implicit none
c         material parameters
        double precision, dimension(6) :: eps6, Deps6, epsn6
        double precision, dimension(3,3) :: eps, epsn, sig
...
!Fill the identity and deviatoriser tensor               
        ...
        Mat_L=1 ! Mat_L=0 : VM, Mat_L=1 : GTN
...
c          Mat_L=0 : VM, Mat_L=1 : GTN
     ...
     &       phitr, epsp_b,eps, epsp, epse, Balpha, alpha,sig_0
     &       ,sig,dsig,f)            
           end if
        end if
        if (Mat_L==0) then
...
c=======================================================================
     ...
     &      epsp_b,eps, epsp, epse, Balpha, alpha, sig_0, sig1,dsig1,f1)
            use tensor_operations
            implicit none
c  Especific variables according to the article N. ARAVAS, 1987
            !Parameters material
...
c           
     ...
     &         A13_PS, D_eps3, dev_e_33, dq_dDeps3
            double precision, dimension(3,3) :: M_ps, M_ps_inv
            double precision, dimension(3) :: b_ps_v, c_corr_ps
            
            !Auxiliar variables
            double precision::conv_D_eps_p_ps, conv_D_eps_q_ps
...
c  Set guess values to enter to the iterative algorithm 
            ...
            D_eps_pn=D_eps_p
            D_eps_qn=D_eps_q
            conv_D_eps_p=1.0000000
            conv_D_eps_q=3.0000000
c=======================================================================
...
c Update Hydrostatic and equivalent stress
            ...
            A_c=f_n/(s_n*(2.0*pi))*DEXP(-1.0/2.0*(epsp_b-E_n)/s_n) !Chu and Needleman parameter A
...
c Derivatives to solve system of equations to solve D_eps_p and D_eps_q 
            dg_dq =2.0*(q/sig_0**2)
            dg_dp = -3.0*q1*q2*f*sinh(-3.0*q2*p/(2.0*sig_0))/sig_0
            d2g_dpdq=0.0
     ...
     &               *cosh((-3.0*q2*p)/(2.0*sig_0))  !
            ...
            !alpha1 = f
            d2g_dpdf=-3.0*q1*q2*p*sinh(-3.0*q2*p/(2.0*sig_0))/sig_0
            d2g_dp_d_epsp=0.0 
            df_d_epsp_b=(A_c/(1.0+D_eps_p))
            d2g_dp_d_epsp_b=d2g_dpdf*df_d_epsp_b
            
            d2g_dqdf=(-4.0*q/sig_0**3)*((-p*D_eps_p+q*D_eps_q)*
     &          (1.0-f)**2/D_epsp_b)
            !alpha2 = epsp (epsilon plastic)
            !alpha3 = f
            d2g_dq_d_epsp=0.0 
            d2g_dq_d_epsp_b= d2g_dqdf*df_d_epsp_b
...
c  !Compute df_dDeps_p
            ...
            dDepsp_dDeps_p=(1.0/3.0)*xid
            dDepsp_dp=contrac_2nd_2nd((D_eps_q*xid),-3.0/2.0*dsig/q**2)
            dDepsp_b_dDeps_p=-p/(1.0-f)/sig_0
            dDepsp_b_dp=-D_eps_p/(1.0-f)/sig_0
c            print*,"dD_dh ", dDf_dDeps_p,dDf_dp,dDepsp_b_dDeps_p,  
c     &          dDepsp_b_dp
            c_f_f = 1.0/(Kron_d(1,1)+D_eps_p)
            !c_f_epsp=0.0 !inv_T_2D(xid)
            c_f_epsp_b=1.0/(Kron_d(1,3)+A_c)
...
c  Compute  d_epsp_b_dDeps_p
            c_epsp_b_f =1.0/(Kron_d(3,1)-(-p*D_eps_p+q*D_eps_q)
     &       /(sig_0*(1.0-f)**2)) 
            !c_epsp_b_epsp =1/(Kron_d(3,2)-(f-1.0)/A_c)
            c_epsp_b_epsp_b=1/(Kron_d(3,3))
...
c  Compute all  dPHI_dH_alpha = dPHI_df, dPHI_d_epsp 
            dPHI_df = 2.0*q1*cosh(-3.0*q2*p/(2.0*sig_0))-2.0*q3*f
            sign_l=(3.0/2.0)**0.5*dsig/(contrac_2nd_2nd(dsig,dsig))
...
c     &            +dPHI_dq*sign_l           !<-------Verify the right operation for the required contractions between Identity matrix
            dPHI_d_epsp_b= (-2.0*q**2/sig_0**3+3.0*f*q1*q2*p*sinh
     &       (-3.0/2.0*q2*p/sig_0)/sig_0**2)*(-(p*D_eps_p+q*D_eps_q)/
     &       ((1.0-f)*D_epsp_b**2))
     
            PHI=(q/sig_0)**2+2.0*f*q1*cosh(-3.0*q2*p/(2.0*sig_0))
     &                -(1.0+q3*f**2)   !<--------------------------------------------Update the value?
...
c     $             +d2g_dp_d_epsp*d_epsp_dDeps_p
            A11=dg_dq+D_eps_p*(xk*d2g_dpdq+sum1)+D_eps_q*(d2g_d2p+sum2)
            A12= dg_dp+D_eps_p*(-3.0*xmu*d2g_d2q+sum1)
c     $             + D_eps_p(-3*G*d2g_dpdq+sum2)
            sum3=dPHI_df*df_dDeps_p+dPHI_d_epsp_b*d_epsp_b_dDeps_p
c     $              +dPHI_d_epsp*d_epsp_dDeps_p
            A21=xk*dPHI_dp+sum3
            sum4=dPHI_df*df_dDeps_q+dPHI_d_epsp_b*d_epsp_b_dDeps_q
c     $            +dPHI_d_epsp*d_epsp_dDeps_q
            A22=-3.0*xmu*dPHI_dq+sum4
            b1=-D_eps_p*dg_dq-D_eps_q*dg_dq
            b2= -PHI
...
c Update Hydrostatic and equivalent stress
       ...
       !else
           ...
           !   D_eps22=D_eps(2,2)     
           !   D_eps3=D_eps(3,3)
           !   pe=-xk*(Trace(eps)+D_eps11+D_eps22)
           !   p_tr=pe-xk(D_eps3-D_eps_p)
           ...
           !   depse=epsen-1.0/3.0*Trace(epsen)*xid !In the article is called e^e, deviatoric elastic eps
           ...
           !   da=a-1.0/3.0*Trace(a)*xid
           !   dsig=se+2.0*G*(D_eps3*da-3.0/2.0*D_eps_q/q*dsig)
           !   q=-3.0*xmu*D_eps_q+(q_tr^2+6*xmu*se(3,3)*D_eps3+4*xmu**2*D_eps3**2)**0.5   
